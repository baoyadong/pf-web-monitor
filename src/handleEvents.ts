...
const HandleEvents = {
  ...
  handleHttp(data, type) {
    ...
    // 添加用户行为，去掉自身上报的接口行为
    if (!data.url.includes(options.host)) {
      breadcrumb.push({
        type,
  ...
  handleError(ev) {
    try {
      ...
      if (target.localName) {
        ...
        // 上报接口报错的话，也不上传---基于link的标签请求host地址；
        const { href, localName } = target
        if(localName.toLowerCase() === 'link' && transportData.errorDsn.includes(href)) {
          return;
        }
      ...
      console.error('web-monitor: handleError错误解析异常:', er);
    }
  },
  handleHistory(data) {
    const { from, to } = data;
  ...
  handleUnhandleRejection(ev) {
    try {
      ...
      console.error('web-monitor: handleUnhandleRejection错误解析异常:', er);
    }
  },
  handlePerformance() {
