...
export default {
  ...
  methods: {
    getTableData() {
      setTimeout(() => {
        fetch(`http://localhost:7001/api/getErrorList?projectId=abcd`)
    ...
    playRecord(id) {
      fetch(`http://localhost:7001/api/getRecordScreenId?id=${id}`)
        .then(response => response.json())
        .then(res => {
          let { code, data } = res;
          const originalEvents = data.data && data.data.events
          if (code == 0 && originalEvents) {
            let events = unzip(originalEvents);
